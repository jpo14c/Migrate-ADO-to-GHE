...
4. ACTOR with the user or    team name, and TYPE with USER or TEAM.
   ### gh ado2gh grant-migrator-role --github-org ORGANIZATION --actor ACTOR --actor-type TYPE
   ### Note   If you're the granting the migrator role for GHE.com, you must also include the target API URL for your enterprise's
   subdomain. For example: --target-api-url   https://api.octocorp.ghe.com
6. Run the generate-script command to generate a migration script.
    ### gh ado2gh generate-script --ado-org ORGNAME --github-org ORGNAME --all

7. The previous command will have created a migrate.ps1 PowerShell script. Review the steps in the generated script and tweak if necessary.

8. The migrate.ps1 script requires PowerShell to run. If not already installed see the install instructions to install PowerShell on Windows, Linux, or Mac. Then run the script.
